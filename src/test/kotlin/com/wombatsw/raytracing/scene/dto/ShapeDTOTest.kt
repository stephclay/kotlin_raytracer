...
class ShapeDTOTest {
    ...
    fun `should resolve to Quad`() {
        ...
        val quad = dto.resolve(ctx)

        assertEquals(Point(2, 2, 2), quad.corner)
        assertEquals(Vector(0, 2, 0), quad.u)
        assertEquals(Vector(2, 0, 0), quad.v)

        val mat = quad.material as Lambertian
        val tex = mat.texture as SolidColor
        assertEquals(Color(1.0, 0.0, 0.0), tex.color)
    }
}